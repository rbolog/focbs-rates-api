...
export interface Env {
	...
	API_VERSION: string; // Cache-busting version, update on each deploy
}
...
export default {
	async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
		...
		// Construct the cache key from the URL, including API_VERSION for cache-busting on deploy
		const versionedUrl = new URL(request.url);
		versionedUrl.searchParams.set('_v', env.API_VERSION ?? '0');
		const cacheKey = new Request(versionedUrl.toString(), request.clone());
...
class FocbsApi {
	...
	private async getAll(): Promise<void> {
		...
		if (keys.list_complete) {
			const allRequests: Promise<CurrencyRate | null>[] = [];
			for (const k of keys.keys) {
				allRequests.push(this.getKv<CurrencyRate>(k.name, true));
			}
			const currencies = await Promise.all(allRequests);
			const eTag = await this.getKv<string>('KEY_HASH', true);
	...
	private async getCurrency(): Promise<void> {
		const currencyRate = await this.getKv<CurrencyRate>(this.code!);
		if (currencyRate) {
	...
	private async getPairRate(): Promise<void> {
		...
		let value: number = this.amount ?? 0;
		...
		if (this.from !== this.to) {
			const currencyRate =
				this.from === defaultCurrency ? await this.getKv<CurrencyRate>(this.to!) : await this.getKv<CurrencyRate>(this.from);

			// Inline narrowing so TypeScript can verify non-null access on currencyRate, from & to
			if (currencyRate && this.from && this.to && (this.to === defaultCurrency || this.from === defaultCurrency)) {
				const rate = this.to === defaultCurrency ? currencyRate.rate : 1.0 / currencyRate.rate;
				const rateAmount = this.to === defaultCurrency ? 1.0 / currencyRate.amount : currencyRate.amount;
				value = currency_helper(rateAmount * rate * (this.amount ?? 0)).value;
		...
		if (!(this.response instanceof Response)) {
			const content: PairRate = {
				from: {
					currency: this.from,
					amount: currency_helper(this.amount ?? 0, currency_format).format(),
				},
				to: {
					currency: this.to!,
	...
	private async isCurrencyExist(): Promise<void> {
		const value = await this.getKv<CurrencyRate>(this.code!);
		const content = {
	...
	/**
	 * Read a value from the KV store, optionally bypassing the key prefix.
	 * The generic parameter T defines the expected JSON shape.
	 */
	private getKv<T = unknown>(key: string, noPrefix: boolean = false): Promise<T | null> {
		const keyValue = noPrefix ? key : `${this.env.KEY_PREFIX}${key}`;
		return this.env.KV_CURRENCIES_RATES.get(keyValue, { type: 'json' }) as Promise<T | null>;
	}

	/**
	 * List all keys matching the configured prefix from the KV store.
	 */
	private listKeys() {
